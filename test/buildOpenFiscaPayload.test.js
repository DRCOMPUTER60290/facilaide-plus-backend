...
test("menage depcom defaults to 60100 and honors provided code", () => {
  ...
  const numericPayload = buildOpenFiscaPayload({
    depcom: 13055
  });
  assert.strictEqual(
...
test("tenant households expose rent and charges in menage payload", () => {
  const now = new Date();
  const currentMonth = getCurrentMonthKey(now);

  const tenantPayload = buildOpenFiscaPayload({
    loyer: 780,
    charges_locatives: 120,
    logement: { statut: "locataire" }
  });
  ...
  assert.strictEqual(
    tenantPayload?.menages?.menage_1?.charges_locatives?.[currentMonth],
    120
  );

  const nestedTenantPayload = buildOpenFiscaPayload({
    logement: {
      statut: "locataire_hlm",
      loyer: { montant: "910" },
      charges_locatives: { montant: "85" }
    }
  });
  ...
  assert.strictEqual(
    nestedTenantPayload?.menages?.menage_1?.charges_locatives?.[currentMonth],
    85
  );
  ...
  assert.strictEqual(
    ownerPayload?.menages?.menage_1?.charges_locatives,
    undefined
  );
});

test("free text message extracts rent and housing charges", () => {
  const now = new Date();
  const currentMonth = getCurrentMonthKey(now);

  const payload = buildOpenFiscaPayload({
    message:
      "Je paie un loyer de 500€ et des charge locatative s'élevant à 100 euros par mois.",
    logement: { statut: "locataire" }
  });

  assert.strictEqual(
    payload?.menages?.menage_1?.loyer?.[currentMonth],
    500
  );
  assert.strictEqual(
